...
func TestParseConfig(t *testing.T) {
	...
	if cfg.RootfsType != "ext4" {
		t.Errorf("RootfsType = %q, want ext4", cfg.RootfsType)
	}
}

// The GPU guest is erofs, not ext4, and rootfs_type is the only place that says
// so -- it is not part of kernel_params, so the root parameters cannot simply be
// passed through from the config.
func TestParseConfigRootfsType(t *testing.T) {
	cfg, err := ParseConfig([]byte(`[hypervisor.clh]
rootfs_type = "erofs"
`), 2048, 1)
	if err != nil {
		t.Fatalf("ParseConfig: %v", err)
	...
	if cfg.RootfsType != "erofs" {
		t.Errorf("RootfsType = %q, want erofs", cfg.RootfsType)
	}
}
...
// gpuConfig is what hack/microvm-assets/assemble-gpu.sh emits for the NVIDIA GPU
// guest, verbatim. Upstream ships no clh flavour of its GPU config (kata's own
// GPU support is QEMU-only), so the script derives one, and this asserts the two
// halves still agree: the script writes only the keys ParseConfig reads, with the
// quoting a toml parser expects.
const gpuConfig = `# Generated by hack/microvm-assets/assemble-gpu.sh from kata-static 4.0.0.
...
[hypervisor.clh]
rootfs_type = "erofs"
kernel_params = "cgroup_no_v1=all pci=realloc pci=nocrs pci=assign-busses"
default_memory = 2048
default_vcpus = 1
`

func TestParseGeneratedGPUConfig(t *testing.T) {
	cfg, err := ParseConfig([]byte(gpuConfig), 512, 8)
	if err != nil {
		t.Fatalf("ParseConfig: %v", err)
	}
	if cfg.RootfsType != "erofs" {
		t.Errorf("RootfsType = %q, want erofs (the guest would boot with ext4 root params)", cfg.RootfsType)
	}
	// pci=realloc/nocrs/assign-busses is BAR reallocation and bus reassignment for
	// the passed-through GPU, so losing it is a passthrough failure, not cosmetic.
	if want := "cgroup_no_v1=all pci=realloc pci=nocrs pci=assign-busses"; cfg.KernelParams != want {
		t.Errorf("KernelParams = %q, want %q", cfg.KernelParams, want)
	}
	// The generated file carries sizing, so the caller's defaults must not win.
	if cfg.MemoryMiB != 2048 || cfg.VCPUs != 1 {
		t.Errorf("sizing = %dMiB/%dvcpu, want 2048/1", cfg.MemoryMiB, cfg.VCPUs)
	}
}
